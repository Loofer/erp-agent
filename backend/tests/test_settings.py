...
def test_motorparts_environment_variables_map_to_scoped_fields(monkeypatch) -> None:
    ...
    monkeypatch.setenv("MOTORPARTS_API_BASE_URL", "https://erp.example.test")
    monkeypatch.setenv("MOTORPARTS_API_TOKEN", "erp-token")
    ...
    assert settings.motorparts_api_base_url == "https://erp.example.test"
    assert settings.motorparts_api_token is not None
    assert settings.motorparts_api_token.get_secret_value() == "erp-token"
