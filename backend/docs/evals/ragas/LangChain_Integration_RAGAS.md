...
> Source: https://docs.ragas.io/en/stable/howtos/integrations/langchain/
...
We will now build a RAG‑based system that integrates the retriever, LLM, and prompt into a Retrieval QA Chain. The retriever fetches relevant documents from a knowledge base. The LLM will generate responses based on the retrieved documents; the prompt will guide the model's response, helping it understand the context and generate relevant and coherent language‑based output.
...
To evaluate the Q&A system we need to structure the queries, expected_responses and other metric‑specific requirements into an [EvaluationDataset](https://docs.ragas.io/en/stable/references/evaluation_schema/#ragas.dataset_schema.EvaluationDataset).
...
For more details on these metrics and how they apply to evaluating RAG systems, visit [Ragas Metrics Documentation](https://docs.ragas.io/en/stable/howtos/concepts/metrics/available_metrics/).
...
---
### Quick notes about this official example
1. This example manually calls `retriever.invoke()` outside the LangChain chain to capture `retrieved_contexts` for Ragas. If you use full‑end‑to‑end LangChain RAG chains, you need to intercept intermediate retrieval outputs.
2. `Faithfulness` measures hallucination; `LLMContextRecall` measures retrieval quality; `FactualCorrectness` compares final answer against ground‑truth reference.
3. The evaluator LLM here re‑uses the same `gpt‑4o‑mini` as the RAG generator; in production you may use a stronger judge model.
