...
> 一次演示能证明 Agent 曾经做对过一件事，却不能证明改动之后它仍然做对。ERP Agent 同时依赖知识库检索、工具选择、子 Agent 委派和最终回答，只看回复是否流畅，无法定位问题发生在召回、工具调用还是生成阶段。RAGAS 评估的价值，是把这条链路变成可重复、可诊断的实验。
...
| 回答正确却漏调用 ERP 工具 | 测试数据或模型知识恰好覆盖了答案 | 无法发现生产环境的数据时效风险 |
...
为避免评估产生 ERP 写操作，运行链路替换了生产 HTTP 客户端：
...
生产 Agent 图与 ChatService
  ...
  └─ ERP 工具 ──────────► MockTransport 只读 Fixture
...
[`fixtures/erp.py`](../../evals/fixtures/erp.py) 会记录每个请求，并拒绝 `POST`、`PUT` 等写操作。创建、更新和人工审批（HITL）不在这套离线评估范围内；即使 Agent 路由错误，也不会改变 ERP 状态。

## 三、证据怎样进入 RAGAS

RAGAS 需要看到“回答基于什么”。当前实现不会只传入向量检索片段，也不会只传 ERP 工具结果，而是将两类可观察证据合并：
...
- 主 Agent `erp-agent` 的最终文本；
- 实际发生的工具调用；
- 除 `search_knowdge` 外已完成工具的返回内容；
- Agent 错误或意外 HITL 中断。

把工具返回加入证据，使同一套 RAGAS 指标可覆盖知识问答和只读 ERP 问答。但它也意味着解读指标时必须看任务类别：ERP 查询的 `faithfulness` 表示回答是否被工具返回支持，知识问答的 `faithfulness` 更接近回答是否被检索文档支持。
...
| `tool_correctness` 降、RAG 分数正常 | 文档问答表现稳定，但 ERP 路由或委派发生回归 | 检查主 Agent Memory、子 Agent描述和 Skill 前置流程 |
...
ERP fixture 是固定的，Agent 图和评估集也可版本控制；但 Zilliz/Milvus 集合保持在线。因此集合内容、Embedding 模型、查询改写权重、混合检索参数或 Reranker 改动，都可能改变 RAG 样本的分数。
