...
在汽配 Motorparts 知识库中，用户可能问“P-100 的刹车皮能不能装”，而文档写的是“P-100 制动片适配关系”“刹车片兼容车型”或“制动系统安装规范”。如果只拿原句检索，精确型号或许能命中，但“能不能装”背后的兼容性、适配性、安装条件未必进入候选集；如果用户只问“这个东西咋退”，文档里的“退货申请”“售后流程”“退款规则”又可能全部漏掉。
...
当前实现使用的权重为 `original=1.00`、`keyword=0.90`、`intent=0.85`、`semantic=0.80`。这表达了一个偏保守的判断：Motorparts 场景的实体与约束比泛化语义更容易决定答案正确性。
...
### 需要实时 Motorparts 数据：不要把 RAG 当事实源

“当前库存”“订单是否已发货”“今天的价格”需要调用已注册 Motorparts 工具。改写后的知识库检索可用于找流程、规则和字段含义，不能替代实时业务数据查询。
