...
description: >
  采购订单创建与更新主技能包。完成订单业务信息收集、字段校验、组装合规订单payload，输出交由外部系统处理审批与ERP写入。
---
...
> 说明：审批、ERP写入属于外部系统能力，本Skill仅负责AI侧信息收集、校验、payload组装，不实现审批逻辑，不直接调用ERP。
...
3. **话术约束**：仅输出提交审批类提示；**审批通过且ERP返回成功前，严禁告知用户订单已创建/已更新/写入ERP**。
...
2. 审批驳回、ERP报错属于外部系统事件，本Skill不修改本地`order_draft`，由外部系统完成用户提示。
...
| payload已输出，待外部审批 | “订单信息已整理完成，已提交审批流程。” | “订单已写入ERP。” |
| 审批被拒（外部返回） | “审批未通过，请修改信息后重新提交。” | “订单创建失败。” |
| 审批+ERP全部成功（外部返回） | “订单已成功创建并写入ERP。” | - |
